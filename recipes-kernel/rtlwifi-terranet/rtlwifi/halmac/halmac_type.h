...
#define RTL_CANNOT_RUN(padapter) \
	(halmac_adapter_validate(padapter) || \
	 halmac_api_validate(padapter))
...
__inline static void _up_sema(struct semaphore *sema)
{
	up(sema);
}

__inline static u32 _down_sema(struct semaphore *sema)
{
...
__inline static void thread_enter(char *name)
{
	allow_signal(SIGTERM);
}

__inline static void thread_exit(struct completion *comp)
{
